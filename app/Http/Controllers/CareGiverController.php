...
public function add(Request $req)
{
    ...
    try {
        ...
        $caregiver->external_id = Str::uuid();
        if($caregiver->save()){
            ...
            // if ($req->has('background-check') && $req->hasFile('background_check_file')) {
            //     $profile->background_check = $req->file('background_check_file')->store('caregiver_docs');
            // }

            // if ($req->has('cpr-certification') && $req->hasFile('cpr_certification_file')) {
            //     $profile->cpr_certification = $req->file('cpr_certification_file')->store('caregiver_docs');
            // }

            // if ($req->has('tb-test') && $req->hasFile('tb_test_file')) {
            //     $profile->tb_test = $req->file('tb_test_file')->store('caregiver_docs');
            // }

            // if ($req->has('license') && $req->hasFile('drivers_license_file')) {
            //     $profile->drivers_license = $req->file('drivers_license_file')->store('caregiver_docs');
            // }
            $profile->background_check = $req->hasFile('background_check_file')
                ? $req->file('background_check_file')->store('caregiver_docs')
                : $profile->background_check;

            $profile->crp_certification = $req->hasFile('cpr_certification_file')
                ? $req->file('cpr_certification_file')->store('caregiver_docs')
                : $profile->cpr_certification;

            $profile->tb_test = $req->hasFile('tb_test_file')
                ? $req->file('tb_test_file')->store('caregiver_docs')
                : $profile->tb_test;

            $profile->licence = $req->hasFile('drivers_license_file')
                ? $req->file('drivers_license_file')->store('caregiver_docs')
        ...
        \Log::error('Error adding caregiver: ' . $e->getMessage());
        return redirect()->back()->with('error', 'Failed to add caregiver. ' . $e->getMessage());
    }
}
